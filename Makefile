...
.PHONY: all build test test-race vet fmt-check mod-verify e2e-compile diff-check ci-diff-check check skill-generate skill-check npm-ci npm-test npm-pack-check npm-smoke npm-check release-pack release-verify clean help
...
help:
	@printf '%s\n' \
		...
		'make skill-check            Verify generated Skill references and version synchronization' \
